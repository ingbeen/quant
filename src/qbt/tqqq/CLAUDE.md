...
- 스케일 무관성: 금액과 무관하게 비율 동일 (실제 10 vs 시뮬 9 = 실제 1000 vs 시뮬 900)
...
2. `실제_종가`: 실제 TQQQ 종가
3. `시뮬_종가`: 시뮬레이션 종가
4. `실제_일일수익률`: 실제 일일 수익률 (%)
5. `시뮬_일일수익률`: 시뮬레이션 일일 수익률 (%)
...
1. `rank`: 순위 (누적배수로그차이\_평균 기준 오름차순)
2. `검증기간_시작`: 검증 시작일
3. `검증기간_종료`: 검증 종료일
4. `총일수`: 검증 기간 총 거래일 수
5. `leverage`: 레버리지 배율 (고정: 3.0)
6. `funding_spread`: 자금 조달 스프레드 (%)
7. `expense_ratio`: 연간 운용 비용 비율 (소수)
8. `누적수익률_실제(%)`: 실제 TQQQ 누적 수익률
9. `누적수익률_시뮬레이션(%)`: 시뮬레이션 누적 수익률
10. `누적수익률_절대차이`: 마지막 날 기준 누적수익률 절대차이 (퍼센트포인트)
11. `누적배수로그차이_평균(%)`: **평균 로그 추적오차** (최적화 기준)
12. `누적배수로그차이_RMSE(%)`: RMSE 로그 추적오차
13. `누적배수로그차이_최대(%)`: 최대 로그 추적오차

**용도**: 최적 비용 모델 파라미터 선정, 성능 벤치마킹

**정렬 기준**: `누적배수로그차이_평균(%)` 오름차순 (낮을수록 우수)
