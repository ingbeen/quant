...
def main():
    ...
    with col0:
        demo_type = st.selectbox(
            "Select Demo", options=["StockChart Demo", "Yield Curve Demo", "Multi-Chart Demo"], index=0
        )
    ...
    with col1:
        ...
        selected_theme = st.selectbox("Select Theme", options=list(theme_options.keys()), key="theme_selector")
    ...
    with col3:
        if demo_type == "StockChart Demo":
            chart_style = st.selectbox(
                "Select Price Chart Style", options=["Candlestick", "Bar", "Line"], index=0, key="chart_style_selector"
            )
