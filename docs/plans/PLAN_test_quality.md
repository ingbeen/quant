...
- [ ] 목표 1: `try/except/pass` 패턴으로 미검증되는 테스트 수정 (보고서 A-4)
- [ ] 목표 2: 조건부 가드(`if len >= N`)로 인한 assert 누락 패턴 개선 (보고서 G-1)
- [ ] 목표 3: 타임존 의존적 하드코딩 수정 (보고서 G-2)
- [ ] 목표 4: `caplog` 미활용 테스트 수정 (보고서 G-3)
...
- [ ] A-4: `test_ffr_fallback_within_2_months`가 fallback 동작을 명확히 검증
- [ ] G-1: 조건부 가드 앞에 데이터 길이 assert 추가 (또는 가드 제거 후 직접 assert)
- [ ] G-2: 타임존 의존성 제거, 날짜 부분 매칭 또는 타임존 독립적 검증으로 변경
- [ ] G-3: `caplog` 사용 시 경고 로그 발생 여부 검증 추가, 또는 `caplog` 인자 제거
...
- [ ] A-4: `test_ffr_fallback_within_2_months` 수정
...
- [ ] G-1: `test_strategy.py`의 조건부 가드 개선 (4개 위치)
