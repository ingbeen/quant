...
- 대시보드 앱:
  ...
  - `app_portfolio_backtest.py`: 포트폴리오 실험 비교 대시보드 (Streamlit + Plotly)
    ...
    - 주요 섹션:
      ...
      - 실험별 탭: 요약 지표, 에쿼티+드로우다운 서브플롯, 자산별 비중 추이(목표비중 수평선), 거래 현황 바차트, 거래 내역 테이블, 시그널 차트(자산 선택), 파라미터 정보, 포트폴리오 보유 현황(날짜별 보유종목/비중/수익률), 체결 전후 비교, 리밸런싱 히스토리, 월별 수익률 히트맵, 자산별 수익 기여도
  - `app_portfolio_debug.py`: 포트폴리오 디버그 대시보드 (Streamlit + Plotly)
    ...
    - 주요 섹션: 일별 상태 네비게이터(select_slider), 동기화 시계열 4행 서브플롯(에쿼티/비중/현금/주수), 체결 상세 테이블, 시그널-체결 추적(pending→executed 매칭)
