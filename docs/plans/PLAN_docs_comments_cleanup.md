# Implementation Plan: 문서/주석/설정 정비
...
- 비즈니스 로직 변경: 모든 수정은 문서/주석/설정/docstring에 한정
- "학습 포인트:" 주석 일괄 제거 (E-3): 교육적 가치가 있으므로 이번 범위 외
- 코드 동작 변경: 런타임 동작은 일절 변경하지 않음

## 3) 배경/맥락(Context)

### 현재 문제점 / 동기

- 리팩토링 과정에서 파일명 변경(`buffer_zone.py` → `buffer_zone_helpers.py`)이 문서에 반영되지 않음
- 메타 타입 `tqqq_validation`이 삭제되었지만 docstring/문서에 잔존
- 테스트 주석에 금지된 개발 단계 표현(레드, Phase N)이 남아 있음
- `pyproject.toml`에 사용하지 않는 DuckDB 참조가 남아 있음
- `pytest.ini`의 coverage 설정이 무효할 수 있음

### 영향받는 규칙(반드시 읽고 전체 숙지)

> 아래 문서에 기재된 규칙을 **모두 숙지**하고 준수합니다.

- `CLAUDE.md`(루트): 주석 작성 원칙, 비율 표기 규칙, 상수 명명 규칙
- `tests/CLAUDE.md`: 테스트 문서화 규칙
- `src/qbt/backtest/CLAUDE.md`: 백테스트 도메인 규칙
- `scripts/CLAUDE.md`: 메타데이터 타입 목록
...
**문서/주석 수정**:
- `src/qbt/backtest/types.py` — docstring `buffer_zone.py` → `buffer_zone_helpers.py`
- `src/qbt/utils/meta_manager.py` — docstring `tqqq_validation` 예시 업데이트
- `src/qbt/backtest/strategies/buffer_zone_helpers.py` — 비율 표기 주석 수정 (A-5)
- `tests/CLAUDE.md` — `buffer_zone.py` 참조 수정
- `scripts/CLAUDE.md` — 메타데이터 타입 목록 업데이트
- `src/qbt/tqqq/CLAUDE.md` — `extract_overlap_period` 위치 설명 개선
- `tests/test_strategy.py` — 파일 docstring 정책 불일치(B-4), 금지 표현 제거(E-1)
- `tests/test_meta_manager.py` — 금지 표현 제거(E-1)
- `tests/test_tqqq_simulation.py` — 금지 표현 제거(E-1)
- `scripts/tqqq/spread_lab/app_rate_spread_lab.py` — 변수 표시 혼란 수정(E-2)

**설정 파일 수정**:
- `pyproject.toml` — description에서 DuckDB 참조 제거(B-3)
- `pytest.ini` — minversion 업데이트(H-2), coverage 설정 이동 검토(H-1)
- `pyproject.toml` 또는 `.coveragerc` — coverage 설정 이동(H-1)

### 데이터/결과 영향

- 없음. 문서/주석/설정만 변경하므로 출력 스키마, CSV, JSON에 영향 없음

## 6) 단계별 계획(Phases)

### Phase 1 — 문서-코드 불일치 수정 (그린 유지)
...
1. 프로젝트 / 문서-코드 불일치 해소 및 주석 정비 (동작 변경 없음)
2. 프로젝트 / 문서/주석/설정 정합성 전면 점검 및 수정
3. 프로젝트 / 오래된 문서 참조 업데이트 + 금지 표현 제거
4. 프로젝트 / CLAUDE.md 규칙 준수를 위한 문서/주석/설정 일괄 정비
5. 프로젝트 / docstring, 주석, 설정 파일 정합성 보정

## 7) 리스크(Risks)

- 리스크 매우 낮음: 문서/주석/설정만 변경하므로 런타임 동작에 영향 없음
- coverage 설정 이동(H-1) 시 기존 커버리지 측정 동작이 변경될 수 있으나, 이미 무효한 설정이므로 실질적 영향 없음

## 8) 메모(Notes)

- 이 계획서는 `PROJECT_ANALYSIS_REPORT.md`의 B, E, H, A-5 항목을 대상으로 함
- 모든 변경은 문서/주석/설정에 한정되어 코드 동작에 영향을 주지 않음
