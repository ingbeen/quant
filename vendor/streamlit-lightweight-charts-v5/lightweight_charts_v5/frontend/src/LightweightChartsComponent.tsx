...
function LightweightChartsComponent({
  ...
  useEffect(() => {
    ...
    /**
     * Sixth Phase: Custom Tooltip via subscribeCrosshairMove
     *
     * Reads customValues (Record<string, string>) from the first candlestick
     * series data and displays them in a fixed-position tooltip overlay.
     * customValues are passed from Python and survive JSON serialization.
     */
    ...
    // Find the first candlestick series (main series for customValues)
    let mainCandleSeries: ISeriesApi<"Candlestick" | "Histogram" | "Line" | "Area" | "Bar"> | null = null
    for (let pi = 0; pi < charts.length; pi++) {
      for (let si = 0; si < charts[pi].series.length; si++) {
        if (charts[pi].series[si].type === "Candlestick" && seriesInstances[pi]?.[si]) {
          mainCandleSeries = seriesInstances[pi][si]
          break
        }
      }
      if (mainCandleSeries) break
    }
    ...
    return () => {
      ...
      // Unsubscribe crosshair and remove tooltip
      try {
        chart.unsubscribeCrosshairMove(crosshairHandler)
      } catch (e) {
        console.warn("Chart disposal: Could not unsubscribe crosshair", e)
      }
