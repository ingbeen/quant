...
- WFE Calmar와 WFE CAGR 모두에서 양수 막대가 우세하고, 뚜렷한 음수는 초기 일부 윈도우에 집중됩니다.
- W2(13.2)와 W4(16.1)는 IS Calmar가 0에 가까워 비율이 크게 튄 특수 케이스입니다.
- WFE Robust 0.93은 이런 극단값을 제거한 중앙값으로, IS 성과의 93%가 OOS에서 재현됨을 의미합니다. 양호한 수준이지만, 이것만으로 전략의 robustness를 강하게 단정하지는 않습니다.
- 따라서 QQQ의 WFO는 **보조 검증 근거**로 사용하고, 핵심 판단은 다른 검증 결과와 함께 종합해서 내려야 합니다.

**TQQQ** (현재 결과 기준):

- W0/W1/W2 등 일부 윈도우에는 큰 음수 WFE가 존재하고, W4/W7/W9 같은 일부 윈도우는 강한 양수입니다. 즉 분포가 매우 비대칭입니다.
- W1(-16.6), W2(-97.4), W4(52.6) 같은 값은 IS Calmar가 0 또는 음수에 가까운 구간에서 비율이 크게 튄 예외치입니다. 수치 자체보다 방향과 분포를 보는 것이 중요합니다.
- 극단값을 제거한 WFE Robust가 0.19라는 것은, TQQQ Dynamic의 IS 성과 중 약 19%만 OOS에서 재현된다는 뜻입니다.
- 핵심 결론은 **"동적 변경이 필요하다"가 아니라 "동적으로 바꿔도 OOS 재현성이 낮다"** 입니다. 따라서 동적 재최적화의 가치는 낮고, 실전 기대치는 보수적으로 봐야 합니다."""
    )


# ============================================================
# 섹션 6: 윈도우별 상세 차트 (캔들 + Buy/Sell 마커)
# ============================================================
