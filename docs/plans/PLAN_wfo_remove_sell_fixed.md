...
- [x] WFO 3-Mode(dynamic/sell_fixed/fully_fixed) → 2-Mode(dynamic/fully_fixed)로 전환
- [x] sell_fixed 관련 코드, 상수, CSV 파일, 문서를 모두 제거
- [x] 향후 WFO 시각화 대시보드 구현을 고려한 구조 유지
...
- [x] sell_fixed 관련 CSV 4개 삭제 완료
- [x] `walkforward_summary.json` 2개에서 sell_fixed 섹션 제거 완료
- [x] `run_walkforward.py`에서 sell_fixed 모드 실행/저장/출력 로직 제거
- [x] `constants.py`에서 sell_fixed 관련 상수 제거
- [x] `types.py` docstring에서 sell_fixed 참조 제거
- [x] 문서 업데이트 완료 (`src/qbt/backtest/CLAUDE.md`, `scripts/CLAUDE.md`, `README.md`)
...
- [x] sell_fixed CSV 4개 삭제
- [x] `walkforward_summary.json` 2개에서 `sell_fixed` 키 프로그래밍 제거 (Python으로 JSON 로드 → sell_fixed 삭제 → 저장)
...
- [x] `scripts/backtest/run_walkforward.py` 수정:
  - import에서 sell_fixed 관련 상수 제거
  - Mode 2 (Sell Fixed) 실행 블록 전체 제거
...
- [x] `README.md` 갱신 (sell_fixed 파일명 제거, 3-Mode → 2-Mode)
- [x] 루트 `CLAUDE.md` 확인 — sell_fixed 참조 없음, 변경 불필요
...
- 2026-03-14 16:30: Phase 1~2 완료, 전체 검증 통과 (passed=334, failed=0, skipped=0)
