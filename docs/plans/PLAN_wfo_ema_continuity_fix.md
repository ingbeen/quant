...
- [ ] `run_walkforward()`에서 OOS 평가 시 EMA를 OOS 구간 기준으로 리셋하지 않고, 전체 히스토리 기반 EMA를 유지한다.
- [ ] `_run_stitched_equity()`에서 stitched 자본곡선 생성 시 동일하게 전체 히스토리 기반 EMA를 사용한다.
- [ ] 두 경로가 동일한 EMA 계산 규칙을 따르도록 일관성을 확보한다.
...
- [ ] `run_walkforward()` — OOS 슬라이스 전 전체 signal_df에 모든 MA window 사전 계산
- [ ] `_run_stitched_equity()` — OOS 슬라이스 전 전체 signal_df에 필요한 MA window 사전 계산
- [ ] EMA 연속성 인바리언트를 검증하는 테스트 추가 (전체 히스토리 EMA 슬라이스 == 사전 계산 결과)
...
- [ ] `tests/test_walkforward_schedule.py`에 `TestEmaContiniuty` 클래스 추가
  - [ ] `test_oos_ema_matches_full_history_ema`: 현재 `run_walkforward()`가 OOS에서 EMA를 리셋함을 확인하는 회귀 방지 테스트 (Phase 0에서는 실패 예상)
  ...
  - [ ] `test_stitched_equity_ema_matches_full_history_ema`: `_run_stitched_equity()`도 동일 인바리언트 검증

**Validation (Phase 0)**:

```bash
poetry run pytest tests/test_walkforward_schedule.py::TestEmaContiniuty -v
# 예상: 새 테스트 2개 FAILED (레드 확인)
```
...
- [ ] `src/qbt/backtest/walkforward.py`의 `run_walkforward()` 수정
  - [ ] 윈도우 루프 진입 전, `ma_window_list`의 모든 window에 대해 `signal_df` 전체에 MA 사전 계산
    ```
    # 루프 전: 전체 signal_df에 모든 MA 사전 계산
    signal_df_with_ma = signal_df.copy()
    for ma_window in ma_window_list:
        ma_col = f"ma_{ma_window}"
        if ma_col not in signal_df_with_ma.columns:
            signal_df_with_ma = add_single_moving_average(
                signal_df_with_ma, ma_window, ma_type=DEFAULT_BUFFER_MA_TYPE
            )
    ```
  - [ ] 루프 내 IS/OOS 슬라이스를 `signal_df_with_ma` 기준으로 변경
  - [ ] 루프 내 OOS MA 재계산 블록 제거 (`if oos_ma_col not in oos_signal.columns: ...`)
  - [ ] `is_signal`을 `run_grid_search()`에 전달 시 MA 컬럼이 이미 포함되어 있으므로, `run_grid_search` → `run_buffer_strategy` 내부의 `if ma_col not in signal_df.columns` 가드가 중복 계산을 방지하는지 확인

**Validation (Phase 1)**:

```bash
poetry run pytest tests/test_walkforward_schedule.py::TestEmaContiniuty -v
# 예상: 2개 중 run_walkforward 관련 테스트 PASSED
```
...
- [ ] `scripts/backtest/run_walkforward.py`의 `_run_stitched_equity()` 수정
  - [ ] `params_schedule`에서 필요한 MA window 목록 수집 로직을 OOS 슬라이스 전으로 이동
  - [ ] 전체 `signal_df`에 필요한 MA window 사전 계산 후 OOS 슬라이스 수행
    ```
    # OOS 슬라이스 전: 전체 signal_df에 MA 사전 계산
...
- [ ] `src/qbt/backtest/walkforward.py` — `run_walkforward()` docstring 주석 업데이트 (내부 MA 계산 순서 명시)
- [ ] `README.md` 변경 없음
