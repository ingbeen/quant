...
- 한 줄 요약: **파라미터를 매번 바꾸든 처음 것을 고정하든 결과가 거의 같습니다.** 고정 파라미터(4P)가 충분히 안정적입니다.
- Dynamic CAGR 11.21%와 Fixed CAGR 11.85%는 0.64%p 차이로 사실상 동일합니다. Fixed가 오히려 약간 높은 것은 "파라미터를 바꾸지 않는 것이 더 나을 수 있다"는 것을 시사합니다.
- WFE Calmar Robust가 Dynamic/Fixed 모두 2.09로 동일합니다. IS보다 OOS가 약 2배 좋았다는 뜻으로, IS 추정이 보수적이어서 OOS에서 오히려 더 잘한 것이며, 과최적화 걱정이 없습니다.
- PC 최대 0.44\~0.45는 0.5 미만으로 수익이 비교적 고르게 분산되어 양호합니다.

**TQQQ** (2026.03.15 기준):

- 한 줄 요약: **이전과 달리, Dynamic과 Fixed의 성과가 거의 동일합니다.** 고정 파라미터만으로 충분합니다.
- Dynamic CAGR 24.51%와 Fixed CAGR 26.32%는 Fixed가 약간 높습니다. 이전에는 Dynamic(21.4%)이 Fixed(7.0%)보다 3배 높았으나, recent\_months 제거 후 격차가 완전히 해소되었습니다. 이전의 Fixed 열위는 "고정의 문제"가 아니라 "sell=0.01+동적 조정의 문제"였습니다.
- WFE Calmar Robust가 Dynamic 2.38, Fixed 2.39로 둘 다 높은 양수입니다. 이전 Fixed의 -0.91이 2.39로 반전된 것이 recent\_months 제거의 가장 극적인 효과입니다.
- PC 최대 0.69는 0.5를 초과하여 수익이 특정 윈도우(W9, 2023\~2025 AI 랠리)에 집중되어 있습니다. 레버리지 상품의 구조적 특성이기도 하지만, 해당 구간을 제외하면 CAGR이 상당히 낮아질 수 있어 리스크 요소로도 인식해야 합니다."""
    )
...
- 한 줄 요약: **파란선(Dynamic)과 주황선(Fixed)이 거의 겹쳐서 진행합니다.** "어떤 파라미터 선택 방식을 쓰든 결과가 비슷하다"는 것이 시각적으로 확인됩니다.
- 그래프 읽는 법: 두 선이 함께 올라가고 함께 내려가며, 2020년 이후 약간의 괴리가 생기나 방향은 동일합니다. 전략 구조 자체가 견고하고 특정 파라미터에 의존하지 않는다는 가장 직관적인 증거입니다.

**TQQQ** (2026.03.15 기준):

- 한 줄 요약: **이전에는 Dynamic이 Fixed를 압도했으나, 현재는 두 곡선이 거의 겹칩니다.** 2020년 이후 Fixed가 Dynamic을 추월하는 구간도 있습니다.
- 그래프 읽는 법: 레버리지 특성상 변동 폭이 QQQ보다 크지만, 두 모드의 궤적이 유사하다는 것이 핵심입니다.
- 이전 대시보드에서 "Dynamic이 Fixed를 압도한다"고 보였던 것은 동적 조정(recent_months)이 Fixed를 오염시킨 결과였습니다. 오염 제거 후 TQQQ도 QQQ와 마찬가지로 "Dynamic ≈ Fixed"가 되었습니다."""
    )
...
- 11개 윈도우 대부분에서 OOS CAGR(빨간 막대)이 IS CAGR(초록 막대)보다 높습니다. IS보다 OOS에서 더 잘했다는 뜻으로, 과최적화의 반대 패턴입니다.
- W1에서만 OOS가 음수이나, 이것은 닷컴 붕괴 직후의 시장 환경 문제이지 과최적화가 아닙니다.

**TQQQ** (2026.03.15 기준):

- QQQ와 유사한 패턴입니다. W2에서 OOS CAGR이 극단적으로 높은데(101.46%), 이것은 2009 급반등 구간에 해당합니다.
- W0과 W1은 OOS CAGR이 각각 -6.56%, -25.22%로 음수인데, 이것은 초기 IS 데이터(1999\~2007, 닷컴 버블 포함)의 구조적 한계에 의한 것입니다. Expanding Window WFO에서 초기 IS가 특수한 시장 환경을 포함하면 그 환경에 과적합된 파라미터가 선택됩니다.
- W3 이후로는 전반적으로 OOS가 IS 이상의 성과를 보이며, 전략의 일반화 능력이 양호함을 시사합니다."""
    )
...
- **MA Window = 200:** 11개 윈도우 전체에서 수평. 어떤 시기에 최적화하든 MA=200이 선택됩니다.
- **Buy Buffer:** W0에서 3%, W1에서 1%로 하락, W2부터 3%로 복귀하여 W10까지 유지. W1의 1%는 초기 IS 데이터 부족에 의한 노이즈입니다.
- **Sell Buffer = 5%:** 전체 수평. 가장 안정적인 파라미터입니다.
- **Hold Days:** W0에서 3, W1에서 5, W2에서 3으로 복귀, W4부터 3으로 수렴하여 W10까지 유지.
- 의미: 후반부(W4 이후)가 현재 확정값(MA=200, buy=3%, sell=5%, hold=3)과 정확히 일치합니다. **"시간이 지나면서 자연스럽게 4P로 수렴했다"**는 것이 고정 파라미터의 타당성을 뒷받침합니다.

**TQQQ** (2026.03.15 기준):

- **MA Window:** W0~W1에서 100~150, W2부터 150으로 수렴하여 W10까지 유지. QQQ(200)보다 짧은 MA가 레버리지에 적합합니다.
- **Buy Buffer = 3%:** 전체 수평. QQQ와 동일합니다.
- **Sell Buffer:** W0에서 5%, W1에서 1%로 급락, W2부터 5%로 복귀하여 W10까지 유지. W1의 1%는 초기 IS 데이터(1999~2007, 닷컴)의 구조적 한계입니다.
- **Hold Days:** W0에서 5, W1에서 0으로 급락, W2에서 5, W3부터 3으로 수렴하여 W10까지 유지(W2만 5).
- 의미: recent_months 제거 후 TQQQ 파라미터가 대폭 안정화되었습니다. 이전에는 sell=0.01~0.03으로 불안정했던 것이 sell=0.05로 안정화되었고, W2 이후로는 극단값이 사라지고 고원 내 값으로 수렴하여 QQQ와 동일한 "시간이 지나면서 수렴" 패턴을 보입니다."""
    )
...
- **11개 윈도우 중 9개가 녹색(양수)**으로, 전략이 실전에서도 대체로 잘 작동합니다. W0과 W1만 빨간색(음수)이며, 이것은 초기 윈도우의 IS 데이터 부족 문제입니다.
- W2(21.6)와 W4(14.8)가 극단적으로 높은데, IS Calmar가 낮아(0에 가까워) 비율이 폭발한 특수 케이스입니다.
- WFE Robust 2.09는 이런 극단값을 제거하고 봐도 양호하다는 의미입니다. 과최적화 걱정이 없습니다.

**TQQQ** (2026.03.15 기준):

- W0(-16.5)와 W2(-160.7)가 극단적 음수입니다. 이것은 IS Calmar가 0 또는 음수에 의한 수학적 특이값이므로 수치 자체에 의미가 없습니다. 구체적으로 W2의 IS Calmar가 -0.015(음수)인데 OOS Calmar가 2.40(양수)이라 OOS/IS = -160.7이 된 것입니다.
- 이런 극단값을 제거한 **WFE Robust가 2.38**로 양호하며, 이것이 실질적 판단 기준입니다. IS보다 OOS가 약 2.4배 좋았다는 뜻으로, IS가 보수적으로 학습하여 OOS에서 오히려 더 잘한 것입니다.
- 이전에는 TQQQ WFE에서 W2가 -1832이라는 극단적 음수가 있었고 WFE Robust가 0.94였습니다. 현재는 극단값의 규모가 -160.7으로 축소되었고, WFE Robust가 0.94에서 2.38로 대폭 개선되었습니다."""
    )
