...
- `load_wfo_results_from_csv`: WFO 결과 CSV를 읽어 WfoWindowResultDict 리스트로 반환 (필수 컬럼 검증, 정수 타입 보정, ATR 선택적 컬럼 처리)
...
- `BufferZoneConfig`: 자산별 전략 설정 (strategy_name, display_name, signal_data_path, trade_data_path, result_dir, grid_results_path, override_* 5개, ma_type). frozen=True
...
- ATR 전략(`buffer_zone_atr_tqqq.py`)은 ATR 전용 로직이 있어 독립 모듈로 유지.
