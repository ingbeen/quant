...
- [x] Pandas CSV 로딩 시 parse_dates 파라미터 사용으로 성능 및 가독성 향상
- [x] Streamlit 앱에 mtime 기반 캐싱 적용으로 CSV 파일 변경 자동 감지
- [x] 두 Streamlit 앱의 캐싱 전략 통일
...
- [x] Pandas: `load_stock_data()` 및 TQQQ 로더들에 parse_dates 적용
- [x] Streamlit: daily_comparison 앱에 mtime 기반 캐싱 적용
- [x] 기존 테스트 모두 통과 (동작 변경 없음)
- [x] `poetry run python validate_project.py` 통과 (failed=0, skipped=0; passed=186, failed=0, skipped=0)
...
- [x] `src/qbt/utils/data_loader.py:load_stock_data()` 수정
...
- [x] `src/qbt/tqqq/data_loader.py` 수정
  ...
  - ~~`load_qqq_data()`, `load_tqqq_data()`: COL_DATE 파싱~~ (해당 함수 없음, utils/data_loader.py 사용)
  - `load_ffr_data()`: DATE 컬럼은 문자열 유지 (yyyy-mm 형식)
- [x] 기존 테스트 실행으로 동작 검증

**Validation**:

- [x] `poetry run python validate_project.py` (passed=186, failed=0, skipped=0)
...
- [x] `scripts/tqqq/streamlit_daily_comparison.py` 수정
...
- [x] 두 Streamlit 앱의 캐싱 전략이 동일한 패턴인지 코드 리뷰

**Validation**:

- [x] `poetry run python validate_project.py` (passed=186, failed=0, skipped=0)
- [ ] Streamlit 앱 수동 테스트 (CSV 재생성 후 새로고침으로 반영 확인) - **사용자 확인 필요**
...
- [x] 필요 시 data_loader 함수 docstring 업데이트 (parse_dates 언급) - 주석으로 추가됨
- [x] `poetry run black .` 실행(자동 포맷 적용) - 42 files unchanged
- [x] 전체 플로우 최종 검증
...
- 2026-01-04 15:30: Phase 1 완료 (Pandas parse_dates 적용) - 테스트 186개 통과
- 2026-01-04 15:40: Phase 2 완료 (Streamlit mtime 캐싱 적용) - 테스트 186개 통과
- 2026-01-04 15:45: 최종 검증 완료 및 계획서 Done 처리
  - 모든 DoD 항목 완료
  - 품질 검증 통과 (passed=186, failed=0, skipped=0)
  - Streamlit 앱 수동 테스트는 사용자 확인 필요 (진행 로그 참고)
