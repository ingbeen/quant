...
- 이 문서가 한 번 작성된 이후에는 **체크리스트 업데이트 외에는 수정하지 않는다.**
  - (예: 진행 체크, Done 처리, 날짜 업데이트는 가능)

※ 나머지 운영 규칙(Phase 구성/승인/커밋 메시지 규칙 등)은 `docs/CLAUDE.md`를 따른다.
...
> (커밋 메시지 규칙은 상단의 “커밋 메시지 형식 규칙”을 참고)
