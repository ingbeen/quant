...
- **기본값: 승인 요청 섹션을 plan에 넣지 않습니다.**
- 단, 사용자가 “특정 Phase에 승인 게이트를 넣어달라”고 **명시적으로 요청한 경우에만**,
  해당 Phase 하단에 `승인 요청 (Optional)` 블록을 추가할 수 있습니다.

#### (Optional) 승인 요청 전 필수 게이트 (승인 블록을 쓰는 경우에만 적용)
...
- 사용자가 “중간 Phase 승인/커밋 단위 분리”를 명시적으로 요청하면,
  해당 Phase에 한해 `Commit Messages (Phase N)` 또는 승인 블록을 추가할 수 있습니다.
