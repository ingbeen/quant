...
class TestCalculateRecentBuyCount:
    ...
    def test_count_within_period(self):
        ...
        # 5/10, 6/20만 포함
        assert count == 2, \
            f"최근 3개월(4/15 이후) 매수는 2건이어야 합니다. 실제: {count}"
...
class TestRunBufferStrategy:
    ...
    def test_missing_ma_column(self):
        ...
        assert "ma_5" in str(exc_info.value) or "컬럼" in str(exc_info.value), \
            "MA 컬럼 누락 에러 메시지"
    ...
    def test_insufficient_valid_data(self):
        ...
        assert "유효" in str(exc_info.value) or "부족" in str(exc_info.value), \
            "유효 데이터 부족 에러"
