...
### 알려진 불일치 — 가격 반올림 자릿수 `2026-09-09`

파이썬 규율은 전역 `~/.claude/rules/python.md` 하나로 통일했고, 그 표의 가격 기준은 **4자리 상한**이다.
**그러나 이 저장소의 기존 구현은 6자리를 쓰며, 사용자 결정으로 «그대로 둔다».**

| 지점 | 값 |
|---|---|
| `src/qbt/backtest/constants.py` `ROUND_PRICE` | `6` (중앙 상수) |
| `src/qbt/utils/stock_downloader.py` · `backtest/engines/backtest_engine.py` · `scripts/tqqq/generate_synthetic.py` | 직접 `.round(6)` / `round(…, 6)` |

- **버그가 아니라 «미이행 상태»다.** 발견해도 조용히 고치지 않는다.
- **바꾸면 기존 백테스트 산출물의 수치가 달라진다** — `docs/research/` 의 결과가 그 시점부터 재현되지 않는다.
- **새로 쓰는 코드는 전역 표(4자리 상한)를 따른다.** 기존 값을 소급해 바꾸지 않는다.
- 통일을 실제로 집행하기로 하면 `ROUND_PRICE` 를 바꾸고 위 3개 파일의 직접 호출을 함께 고친 뒤, 영향받는 결과 문서를 재산출해야 한다.
