...
### 2000~2010년 주요 손실 거래에 각 방안 적용 결과 (추정)

| 거래 | 손익률 | ATR 버퍼 | ADX 스위치 | Stop Loss -20% |
|------|--------|----------|-----------|----------------|
| 2000-04 ~ 2000-05 | -49.58% | 차단 가능 | 차단 가능 | -20%로 제한 |
| 2001-12 ~ 2002-02 | -39.38% | 차단 가능 | 차단 가능 | -20%로 제한 |
| 2004-11 ~ 2005-04 | -22.95% | 효과 제한적 | 차단 가능 | -20%로 제한 |
| 2005-07 ~ 2006-05 | -12.28% | 효과 제한적 | 차단 가능 | 해당 없음 |
| 2008-08 ~ 2008-09 | -25.85% | 차단 가능 | 효과 제한적 | -20%로 제한 |

- **ATR 버퍼**: 변동성 극심한 시기(2000~2002, 2008)의 손실 차단에 강함
- **ADX 스위치**: 횡보 구간(2004~2006)의 whipsaw 차단에 강함
- **Stop Loss**: 모든 구간에서 최대 손실을 일정 수준으로 제한

### 권장 적용 순서

```
1단계: ATR 적응형 버퍼 (고정 buffer_zone_pct 교체)
  → 가장 근본적인 문제(고정 버퍼) 해결
  → 현재 전략 구조와 호환성 높음

2단계: ADX 모드 스위치 (매수 조건에 ADX >= 20 추가)
  → 횡보 구간 거래 자체를 구조적으로 차단
  → ATR 버퍼와 독립적이므로 동시 적용 가능

3단계: Stop Loss (선택 사항)
  → 위 두 방안으로도 걸러지지 않는 예외 상황 대비
  → 안전망 역할
```
