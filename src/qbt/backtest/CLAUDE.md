...
- `SignalStrategy`: `check_buy(prev_close, cur_close, prev_upper, cur_upper, hold_state, hold_days_required) -> tuple[bool, HoldState | None]` + `check_sell(prev_close, cur_close, prev_lower, cur_lower) -> bool`

TypedDict:

- `HoldState`: hold_days 상태머신 상태 (days_held, entry_close, entry_upper)
...
- `BufferStrategyResultDict`: `run_buffer_strategy()` 반환 타입 TypedDict
...
- `BuyAndHoldStrategy`: `SignalStrategy` Protocol 구현체
...
export 심볼: `BufferZoneConfig`, `resolve_params_for_config`, `BufferStrategyParams`, `PendingOrder`, `PendingOrderConflictError`, `BuyAndHoldConfig`, `BuyAndHoldParams`, `create_runner`, `run_buy_and_hold`
