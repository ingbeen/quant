...
6. `종가_상대차이(%)`: 종가 상대차이 (실제 기준, `= ((종가_시뮬 - 종가_실제) / 종가_실제) × 100`)
7. `누적수익률_실제(%)`: 실제 TQQQ 누적 수익률
8. `누적수익률_시뮬레이션(%)`: 시뮬레이션 누적 수익률
9. `누적수익률_상대차이(%)`: 누적수익률 상대차이 (실제 기준, `= ((누적수익률_시뮬 - 누적수익률_실제) / 누적수익률_실제) × 100`)
10. `누적배수로그차이_평균(%)`: **평균 로그 추적오차** (최적화 기준)
11. `누적배수로그차이_RMSE(%)`: RMSE 로그 추적오차
12. `누적배수로그차이_최대(%)`: 최대 로그 추적오차
