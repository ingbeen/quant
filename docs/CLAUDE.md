...
#### 예외: 정말 불가피한 경우에만 스킵 허용

- 스킵이 필요하면 허용할 수 있으나, **스킵이 1개라도 남아있으면**:

  - plan 상태를 ✅ Done으로 처리할 수 없습니다.
  - DoD 체크박스(특히 테스트/Validation 관련)를 [x]로 처리하면 안 됩니다.
  - Notes에 스킵 사유/해제 조건/후속 plan을 반드시 기록합니다.

### 4) 승인(Approval) 규칙

- Phase 2 이상이 존재하면, **Phase 전환 시점마다 승인 요청 블록**이 있어야 합니다.

#### 승인 요청 전 필수 게이트

승인 요청을 올리기 전, 아래를 **반드시** 수행/기록합니다.

- (필수) 해당 Phase 체크박스를 plan에서 최신 상태로 업데이트([ ]→[x])
- (필수) 이번 Phase에서 [x]로 바뀐 항목 목록 요약
- (필수) Validation 결과 요약(실행 커맨드 + 통과/실패 + passed/failed/skipped 수)

> 체크박스 업데이트 없이 승인 요청을 하면 안 됩니다.

### 5) “완료(Done)” 선언 기준 (서술 금지, 체크리스트 기반)

- Done은 “말/요약”이 아니라 **plan의 체크리스트 상태**로만 판단합니다.
- 스킵이 남아있으면 Done 처리 금지입니다.
...
- **승인 요청(Approval)이 있는 모든 지점**에는 `Commit Messages (Phase N)` 섹션을 반드시 둡니다.
- **완료 처리되는 지점**(마지막 Phase 또는 Done 선언 시점)에는 `Commit Messages (Final)`을 반드시 둡니다.
- 전체 계획에 승인 요청이 없다면:

  - 중간 Phase Commit Messages는 생략 가능
  - 완료 지점에 `Commit Messages (Final)`만 있어도 됩니다.
