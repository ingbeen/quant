...
2. **포맷팅 유틸리티 사용**: `qbt.utils.formatting` 모듈의 `format_cell()`, `format_row()` 활용
3. **정렬 방향 지정**: `Align` Enum (LEFT, RIGHT, CENTER) 사용

**주의사항:**
- 컬럼 폭은 들어갈 내용의 최대 터미널 폭보다 크게 설정
- 숫자 데이터는 우측 정렬 (`Align.RIGHT`)이 일반적
- 텍스트 데이터는 좌측 정렬 (`Align.LEFT`)이 일반적
- 들여쓰기(`indent`)는 기본 2칸 사용
