...
(※ 이 가로 영역은 **삭제하거나 수정할 수 없습니다.** 또한 스킵이 남아있는 경우 **Done 처리 또는 DoD 체크가 금지됩니다.**)
...
- [ ] (직전 Phase) 체크리스트 최신화(작업 내용/Validation 체크 상태 반영)
- [ ] DoD 체크리스트 최신화(현재까지 완료된 항목 반영)
- [ ] 스킵 존재 여부 확인(스킵이 있으면 사유/해제 조건/후속 plan 기록)
