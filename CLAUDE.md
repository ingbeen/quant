...
**현재 상태**: 데이터 다운로드 기능에 집중한 간결한 프레임워크 (191줄)
**향후 계획**: 백테스팅 기능 추가 예정
...
- **예외 전파**: 비즈니스 로직에서 발생한 예외는 로깅 후 재발생시켜 호출자에게 전파
...
- **단위 테스트**: pytest 사용 예정
- **테스트 커버리지**: 핵심 비즈니스 로직 우선
- **통합 테스트**: API 호출 부분 모킹 처리

## Git 워크플로우

### .gitignore 정책

- Python 바이너리 제외 (`__pycache__`, `*.pyc`)
- 환경 변수 파일 제외 (`.env`)
- 데이터베이스 파일 제외 (`*.db`, `*.duckdb`)
- IDE 설정 제외 (`.vscode/`, `.idea/`)
