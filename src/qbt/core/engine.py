...
class BacktestEngine:
    ...
    def run_backtest(
        ...
        for idx, row in data.iterrows():
            ...
            if strategy.check_sell_condition(data_series, current_date):
                ...
                if current_position > 0:
                    ...
                    if result["success"]:
                        print(
                            f"[SELL] {current_date}: {result['quantity']:.2f}주 @ ${result['price']:.2f}"
                        )
            ...
            if strategy.check_buy_condition(data_series, current_date):
                ...
                if position_size > 0:
                    ...
                    if result["success"]:
                        print(
                            f"[BUY] {current_date}: {result['quantity']:.2f}주 @ ${result['price']:.2f}"
                        )
        ...
        if final_position > 0:
            ...
            if result["success"]:
                print(
                    f"[FINAL SELL] {final_date}: {result['quantity']:.2f}주 @ ${result['price']:.2f}"
                )
