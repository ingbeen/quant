    ...
    def pane(self) -> Dict[str, Any]:
        ...
        if self.style == "Area":
            ...
            series_config = {
                ...
                "options": {"color": self.theme.primary_line_color, "lineWidth": 2, "priceLineVisible": False},
            }
        ...
        return {"chart": chart_config, "series": series_configs, "height": self.height, "title": display_title}
...
class VolumeIndicator(Indicator):
    ...
    def pane(self) -> Dict[str, Any]:
        ...
        volume_data["color"] = np.where(
            volume_data["open"] <= volume_data["close"], "rgba(38,166,154,0.8)", "rgba(239,83,80,0.8)"
        )
        volume_records = (
            volume_data[["time", "volume", "color"]].rename(columns={"volume": "value"}).to_dict(orient="records")
        )
