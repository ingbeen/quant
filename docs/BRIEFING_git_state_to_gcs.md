...
**예산 메일 알림 (50% / 90% / 100%) + 수동 차단** — 자동 차단 / 코드 임계치 모두 도입하지 않음

근거 (2026-05-09 사용자 최종 결정):

- 본 프로젝트의 데이터 규모(~5MB) 가 무료 한도(5GB) 대비 1/1000 미만이라 청구
  발생 가능성 자체가 매우 낮다.
- 자동 차단 (Pub/Sub + Cloud Function) 은 GCP 인프라 작업 / 결제 권한 부여 / 코드
  배포가 필요해 본 프로젝트 규모 대비 과다 방어로 판단.
- 코드 레벨 임계치 검사 (`storage_gateway` 호출 횟수 / 누적 사이즈 상한) 도 외부
  공격 시나리오 (자격증명 유출 등) 는 막지 못해 자동 차단의 단순 대체가 되지 못한다.
  본 프로젝트의 service account 자격증명은 GitHub Secret 외 노출 경로가 없어 외부
  공격 가능성은 매우 낮다.
- 따라서 청구 보호는 §4 4단계 예산 메일 알림에 위임하고, 100% 도달 시 사용자가
  메일 인지 후 GCP 결제 콘솔에서 직접 결제 계정을 비활성화하는 수동 운영으로
  단순화한다.

설계:

- **사전 인지**: §4 4단계 예산 메일 알림 (50% / 90% / 100% 도달 시 결제 관리자
  이메일 발송).
- **사후 대응**: 사용자가 메일 받고 GCP 결제 콘솔에서 결제 계정 비활성화 클릭.
  자동화 인프라는 두지 않는다.
- **알림 채널 (별도 운영)**: 매매 신호 / 데일리런 실패 알림은 기존 인프라가 그대로
  유지된다. 청구 차단 정책과 별개로:
  - Layer 1 — `cli.py` `main()` 의 `_safe_notify_failure` 훅: `run-daily` 예외를
    FCM + 텔레그램으로 발송 (현행 그대로).
  - Layer 2 — `daily_run.yml` 의 `notify-failure` job (`if: failure()`): Layer 1
    이 작동 못하는 케이스의 out-of-process 백업 (현행 그대로).

수용한 리스크 (사용자 명시 동의):

- 청구가 발생한 시점부터 사용자가 메일 인지 후 차단 클릭하기까지의 지연 동안 추가
  청구 가능. 단 5GB 무료 한도 안에서는 청구 자체가 0 이므로 사실상 영향 없음.
- 자격증명 유출 / 외부 공격 시나리오는 본 프로젝트 환경에서 가능성이 매우 낮아
  수용. 발생 시 메일 알림으로 인지 → 수동 차단.
...
- 4단계 예산 알림까지 진행하면 메일로 사전 인지 가능. 100% 도달 시 사용자가 콘솔에서
  수동으로 결제 비활성화 (§3.5 정책).
...
- 본 프로젝트는 §3.5 결정에 따라 자동 차단 / 코드 임계치를 도입하지 않으며,
  100% 도달 시 사용자가 메일을 받고 GCP 결제 콘솔에서 직접 결제 계정을 비활성화한다.
- 따라서 4단계 완료 = 청구 보호 인프라 완성. 별도 후속 단계 불필요.
...
- **기존 실패 알림 인프라는 그대로 유지** (§3.5 결정의 알림 채널):
  - Layer 1 — `cli.py` `main()` 의 `_safe_notify_failure` 훅: 코드 변경 없음.
  - Layer 2 — `daily_run.yml` 의 `notify-failure` job (`if: failure()`): 코드 변경 없음.
  - 결제 차단 시나리오에서도 위 두 레이어가 그대로 작동하여 텔레그램으로 실패
    알림이 도달한다 (FCM 은 Firebase 정지로 실패).
  - "알림 폭탄 방지" 마커 처리는 **도입하지 않는다** (§3.5 부작용 인정 항목 참조).
...
| Blaze 한도 초과로 청구 발생                   | 코드 버그로 무한 루프 업로드                   | 4단계 예산 알림 (50%/90%/100% 메일) + 사용자 메일 인지 후 콘솔에서 수동 차단 (§3.5) + 코드 리뷰     |
| 외부 공격 / 자격증명 유출                     | service account 키 노출 시 외부에서 burst 작업 | 본 프로젝트 키는 GitHub Secret 외 노출 경로 없어 가능성 매우 낮음. 발생 시 4단계 메일 알림으로 인지  |
...
16. AI: 본 문서에 §3.5 / 5단계 / §6.7 GH Actions failure 알림 / §8 리스크 / §10.4 /
    §11 갱신 (2026-05-08)
17. 사용자 (2026-05-09): "알림 폭탄 방지 마커 처리는 하지 마라" + "데일리런 실패 시
    FCM 과 텔레봇이 이미 발송되고 있는지 코드 분석 요청"
18. AI: 코드 분석 결과 — `cli.py` `main()` 의 `_safe_notify_failure` 훅(Layer 1) +
    `daily_run.yml` 의 `notify-failure` job(Layer 2) 이 이미 FCM + 텔레그램 동시
    발송을 구현 중. 결제 차단 시나리오에서도 텔레그램은 GCP 외부라 도달함 (FCM 은
    실패). §3.5 / §4 5단계 주의사항 / §6.7 / §8 리스크 표 표현을 "신규 추가" →
    "기존 인프라 재사용" 으로 정정. 마커 처리 단락 삭제.
19. 사용자 (2026-05-09 후속): "GCP 에서 그냥 결제 막는건 못하는거야? 왜 소스코드
    구현?" 푸시백
20. AI: GCP 의 결제 자동 차단 설계 한계 (의도적으로 원클릭 미제공, Pub/Sub +
    Cloud Function 이 공식 권장 패턴) 설명. 옵션 A (자동 차단) / B (수동 차단) /
    C (코드 임계치) 비교 제시
21. 사용자: "임계치도 하지 말자, 5단계는 스킵" — 자동 차단 / 코드 임계치 모두 포기,
    메일 알림 + 수동 차단으로 단순화 결정
22. AI: §3.5 정책 자체 교체 ("자동 차단" → "메일 알림 + 수동 차단") / §4 4단계 주의사항
    정정 / §4 5단계 (Pub/Sub + Cloud Function) 전체 삭제 / 기존 6단계 → 5단계 번호
    이동 / §5 사용자 공유 정보 표 정리 / §8 리스크 표 자동 차단 관련 행 정리 /
    §10.4 / §11 갱신
...
- 2026-05-08: 자동 차단 정책 추가
  ...
  - §8 리스크 표 — 자동 차단 관련 신규 리스크 3건 추가, "Blaze 한도 초과" 완화책에
    자동 차단 항목 추가
- 2026-05-09: 코드 분석 후 알림 채널 표현 정정 + 마커 처리 미도입 확정
  - §3.5 — "차단 인지 경로" 를 현재 시스템의 Layer 1 (in-process) + Layer 2
    (GH Actions `notify-failure` job) 이중 안전망 설명으로 정정. "알림 위치" 의
    이유 3 표현을 "이미 사용 중" → "이미 구현 완료" 로 정확화. "부작용 인정" 의
    알림 폭탄 마커 필요성 문장을 "마커 미도입" 정책으로 교체.
  - §4 5단계 주의사항 — `if: failure()` 텔레그램 알림 스텝 dry-run 언급 제거,
    Pub/Sub mock 메시지 발행 검증 방식으로 교체.
  - §6.7 — "if: failure() 텔레봇 알림 스텝 추가" 표현 정정. 현재 워크플로우
    (`daily_run.yml`) 에 이미 `notify-failure` job 이 존재하므로 "기존 인프라
    그대로 유지" 로 명시. "알림 폭탄 방지 마커 처리 후보" 단락 전체 삭제.
  - §8 리스크 표 — "자동 차단 후 알림 폭탄" 행을 "텔레그램 알림 매일 반복" 로
    정정, 완화책을 "마커 미도입 — 결제 복구로 자연 정상화" 로 갱신.
  - §10.4 — 항목 17, 18 추가 (코드 분석 / 표현 정정 흐름).
  - §11 — 본 항목 추가.
- 2026-05-09 (후속): 자동 차단 / 코드 임계치 모두 포기, 메일 알림 + 수동 차단으로 단순화
  - §3.5 — 정책 자체 교체. "결제 자동 차단" → "예산 메일 알림 (50%/90%/100%) +
    수동 차단". 본 프로젝트 데이터 규모(~5MB) 가 무료 한도(5GB) 대비 1/1000 미만이라
    자동 차단 / 코드 임계치 모두 과다 방어로 판단. 자격증명 외부 노출 경로 없어
    외부 공격 가능성도 매우 낮음.
  - §4 도입부 — "코드 변경에 앞서 6단계가 모두 완료" → "5단계가 모두 완료" 로 정정.
  - §4 1단계 주의사항 — "5단계 자동 차단까지 진행하면 사고 위험 제로화" → "4단계
    예산 알림으로 사전 인지 + 사용자 수동 차단 (§3.5)" 로 정정.
  - §4 4단계 주의사항 — "5단계 자동 차단 인프라 연계" 안내 제거, "4단계 완료 =
    청구 보호 인프라 완성" 명시.
  - §4 5단계 (Pub/Sub + Cloud Function) — **전체 삭제**.
  - §4 기존 6단계 (Service Account 권한 확인) — **5단계로 번호 이동**.
  - §5 사용자 공유 정보 표 — "Pub/Sub 토픽 + Cloud Function 자동 차단 설정 여부" 행
    제거, Service Account 행의 "6단계 결과" → "5단계 결과" 로 정정.
  - §8 리스크 표 — 자동 차단 관련 행 3건 ("텔레그램 알림 매일 반복", "자동 차단 잘못된
    트리거", "자동 차단 시 RTDB/FCM 동반 정지") 삭제. "Blaze 한도 초과" 완화책에서
    자동 차단 항목 제거, 메일 알림 + 수동 차단으로 정정. "외부 공격 / 자격증명 유출"
    행 신규 추가. Service Account 행의 "6단계 사전 확인" → "5단계 사전 확인" 정정.
  - §10.4 — 항목 19~22 추가 (자동 차단 / 임계치 모두 포기 결정 흐름).
  - §11 — 본 항목 추가.
