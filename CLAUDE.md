...
- 예: 특정 모듈의 DISPLAY_* 상수, 스크립트 전용 DEFAULT_* 상수
- 코드 근접성 향상으로 가독성 개선
...
- 병렬 유리: 그리드 서치 (1회 Pool 생성, 840개 Python 루프 작업 분배)
...
각 전략의 결과는 전략명 하위 폴더에 저장된다 (예: `buffer_zone_tqqq/`, `buffer_zone_atr_tqqq_wfo/`, `buffer_zone_qqq/`, `buy_and_hold_qqq/`, `buy_and_hold_tqqq/`).
