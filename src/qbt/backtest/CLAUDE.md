...
- `TradeRecord`: 거래 기록 TypedDict
- `EquityRecord`: equity 기록 TypedDict

공통 함수:

- `execute_buy_order(order, open_price, execute_date, capital, position) -> tuple`
- `execute_sell_order(order, open_price, execute_date, capital, position, entry_price, entry_date, hold_days_used) -> tuple`
  - `hold_days_used`: 실제 보유 기간 (호출자가 명시적으로 전달, 암묵적 덮어쓰기 계약 제거)
...
- `run_backtest(strategy, signal_df, trade_df, params, log_trades, strategy_name, params_schedule) -> tuple[trades_df, equity_df, summary]`: SignalStrategy 의존성 주입 방식 실행
- `run_grid_search(strategy_cls, signal_df, trade_df, ...) -> pd.DataFrame`: 파라미터 그리드 탐색 (병렬 처리, WORKER_CACHE 패턴)
...
- `SignalStrategy`: `check_buy(prev_close, cur_close, prev_upper, cur_upper, hold_state, hold_days_required, current_date, buy_buffer_pct) -> tuple[bool, HoldState | None]` + `check_sell(prev_close, cur_close, prev_lower, cur_lower) -> bool`
  - `current_date`: 현재 날짜 → `HoldState.start_date` 직접 설정 (플레이스홀더 `date.min` 방지)
  - `buy_buffer_pct`: 매수 버퍼 비율 → `HoldState.buffer_pct` 직접 설정 (플레이스홀더 `0.0` 방지)
...
- `BufferZoneStrategy`: `SignalStrategy` Protocol 구현체
  - `check_buy(...)`: hold_days 상태머신 포함 (hold_days_required=0이면 즉시 detect_buy_signal 사용)
  - `check_sell(...)`: detect_sell_signal 결과 반환
...
- `BuyAndHoldStrategy`: `SignalStrategy` Protocol 구현체 (명시적 상속)
  - `check_buy(...)`: 항상 `(True, None)` 반환 (파라미터 무시, 즉시 매수)
  - `check_sell(...)`: 항상 `False` 반환 (매도 없음)
...
export 심볼: `BufferZoneConfig`, `resolve_params_for_config`, `BufferStrategyParams`, `PendingOrder`, `PendingOrderConflictError`, `BuyAndHoldConfig`
...
- `create_buffer_zone_runner(config: BufferZoneConfig) -> Callable[[], SingleBacktestResult]`: 버퍼존 전략 runner 팩토리. 데이터 로딩, overlap 처리, MA 계산, `run_backtest(BufferZoneStrategy(), ...)` 수행
- `create_buy_and_hold_runner(config: BuyAndHoldConfig) -> Callable[[], SingleBacktestResult]`: B&H 전략 runner 팩토리. `run_backtest(BuyAndHoldStrategy(), ...)` 기반으로 실행 (엔진 기반 통일). dummy `BufferStrategyParams`(ma_window=1, buy_buffer=MIN, sell_buffer=MIN, hold_days=0) 사용, 결과 signal_df에서 ma 컬럼 제거
