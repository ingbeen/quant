...
- [ ] 기능 요구사항 충족
  - [ ] equity/final_capital이 "현금+평가액" 기준으로 일관되게 계산됨
  - [ ] pending_order 단일 슬롯 정책 구현
  - [ ] pending 중 신규 신호 발생 시 즉시 예외 발생 + 중단
  - [ ] hold_days 타임라인이 예시와 정확히 일치
- [ ] 회귀/신규 테스트 추가
  - [ ] equity 정의 테스트
  - [ ] final_capital 정의 테스트
  - [ ] hold_days=0/1 타임라인 테스트
  - [ ] 마지막 날 규칙 테스트
  - [ ] **Critical Invariant 테스트 (pending 충돌 시 예외 발생)**
- [ ] `./run_tests.sh` 통과
- [ ] `poetry run ruff check .` 통과
- [ ] `poetry run black .` 실행 완료 (마지막 Phase에서)
- [ ] 필요한 문서 업데이트
  - [ ] `docs/CLAUDE.md`: archive 기본 무시 명시
  - [ ] `src/qbt/backtest/CLAUDE.md`: 실행 규칙 상세화
...
- [ ] 기능 단위 함수 분해 (`src/qbt/backtest/strategy.py`)
  - [ ] `_validate_buffer_strategy_inputs()`: 파라미터 검증 (line 587-612 분리)
  - [ ] `_compute_bands()`: 밴드 계산 (line 711-717 분리)
  - [ ] `_execute_pending_at_open()`: pending 실행 (line 679-709 분리)
  - [ ] `_check_pending_conflict()`: invariant 체크 (신규 신호 전 호출)
  - [ ] `_maybe_create_buy_pending()`: 매수 pending 생성 조건 검사 및 생성 (line 739-756 분리)
  - [ ] `_maybe_create_sell_pending()`: 매도 pending 생성 조건 검사 및 생성 (line 758-773 분리)
  - [ ] `_record_equity()`: equity 기록 (line 719-735 분리)
  - [ ] `_finalize_summary()`: 요약 지표 계산 (line 820-825 분리)
  - [ ] 주의: hold_tracking 상태 관리 필요 시 별도 함수 또는 dataclass 추가

- [ ] 타입 안정성 강화
  - [ ] `order_type`을 `Literal["buy", "sell"]`로 변경
    - `PendingOrder` dataclass의 `order_type` 필드 타입 힌트 업데이트
  - [ ] 또는 `OrderType` Enum 도입 (더 안전)
    ```python
    ...
    class OrderType(str, Enum):
        ...
        SELL = "sell"
    ```
  - [ ] hold_tracking 상태가 복잡하면 dataclass 도입
    ```python
    @dataclass
    class HoldTracking:
        start_date: date
...
- [ ] 주석 및 docstring 개선
  - 분해된 함수마다 명확한 docstring (Google 스타일)
  - 복잡한 로직은 인라인 주석 (넘버링)
  - AI가 이해하기 쉬운 명확한 변수명 사용

- [ ] 기존 테스트 유지
  - 리팩토링 중 테스트가 깨지지 않도록 확인
  - 각 함수 분해 후 즉시 테스트 실행
...
- [ ] `./run_tests.sh`
- [ ] 코드 가독성 자체 검토 (함수가 너무 작거나 너무 크지 않은지)

---

### 승인 요청 + Commit Messages (Phase 2)

- [ ] 마지막 Phase로 진행해도 되는지 승인 요청
...
- [ ] `docs/CLAUDE.md` 업데이트
  - [ ] "docs/archive 정책" 섹션 추가
  - [ ] 내용: "docs/archive는 과거 문서 보관용입니다. 현재 작업의 규칙/근거/소스 오브 트루스(Source of Truth)로 사용하지 않습니다. 현행 규칙은 **루트 CLAUDE.md + 각 도메인 CLAUDE.md + 최신 plan**만을 기준으로 합니다."

- [ ] `src/qbt/backtest/CLAUDE.md` 최종 검토
  - [ ] "체결 타이밍 규칙" 섹션 상세화
    - equity/final_capital 정의 명시
    - pending 단일 슬롯 정책 명시
    - Critical Invariant 명시 (pending 중 신규 신호 발생 시 중단)
    - hold_days 타임라인 예시 (0/1/일반화)
    - 마지막 날 규칙 명시
  - [ ] 기존 내용과 충돌하는 부분 수정

- [ ] `tests/test_strategy.py` 최종 검토
  - [ ] 모든 새 테스트에 명확한 docstring 있는지 확인
  - [ ] Given-When-Then 패턴 준수 확인
  - [ ] 주석 가독성 확인

- [ ] 불필요 파일/산출물 정리
  - 임시 파일, 디버그 로그, 주석 처리된 오래된 코드 제거
