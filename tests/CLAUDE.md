...
**정밀도 가이드라인** (프로젝트 실제 사용 패턴):

- **비율 값 (0~1 범위)**: 항목별 소수점 자릿수
  - FFR (연방기금금리): **3자리** (예: `0.045` = 4.5%)
  - Funding Spread: **4자리** (예: `0.004` = 0.4%, 증분: `0.0001`)
  - Expense Ratio (운용비율): **4자리** (예: `0.0095` = 0.95%)
  - Buffer Zone: **2자리** (예: `0.03` = 3%)
  - Slippage: **3자리** (예: `0.003` = 0.3%)

- **가격 값**:
  - 종가/초기자본: **1~2자리** (예: `100.0`, `10000.0`)
  - Leverage 배수: **1자리** (예: `3.0`)

- **성과 지표** (CSV 저장용, % 단위):
  - CAGR/MDD/승률: **2자리** (예: `22.47%`)
  - 누적배수 로그차이: **4자리** (정밀도 중요, 예: `0.0123%`)
