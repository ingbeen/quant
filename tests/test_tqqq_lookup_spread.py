...
class TestBuildSegmentTable:
    """구간별 고정 스프레드 테이블 생성 테스트"""

    def test_three_segments_mean_aggregation(self):
        """
        목적: boundaries=[0,2,4]로 3구간 분할 시 mean 스프레드가 정확한지 검증

        Given: 금리 0~2% 구간에 스프레드 [0.002, 0.004],
               2~4% 구간에 [0.006], 4%+ 구간에 [0.008, 0.010]
        When: build_segment_table(boundaries=[0,2,4], stat_func="mean")
        Then: 각 구간의 평균 스프레드가 정확
        """
        from qbt.tqqq.lookup_spread import build_segment_table

        # Given
        realized_df = pd.DataFrame(
            {
                "ffr_pct": [0.5, 1.5, 3.0, 4.5, 5.5],
                "realized_spread": [0.002, 0.004, 0.006, 0.008, 0.010],
            }
        )

        # When
        table = build_segment_table(realized_df, boundaries=[0.0, 2.0, 4.0], stat_func="mean")

        # Then: 3구간 [(0,2), (2,4), (4,inf)]
        assert table[(0.0, 2.0)] == pytest.approx(0.003, abs=1e-10)
        assert table[(2.0, 4.0)] == pytest.approx(0.006, abs=1e-10)
        assert table[(4.0, float("inf"))] == pytest.approx(0.009, abs=1e-10)

    def test_three_segments_median_aggregation(self):
        """
        목적: median 통계량의 정확성 검증

        Given: 0~2% 구간에 스프레드 [0.001, 0.003, 0.010] (이상치 포함)
        When: build_segment_table(boundaries=[0,2,4], stat_func="median")
        Then: 중앙값 = 0.003 (이상치에 강건)
        """
        from qbt.tqqq.lookup_spread import build_segment_table
        ...
        table = build_segment_table(realized_df, boundaries=[0.0, 2.0, 4.0], stat_func="median")

        assert table[(0.0, 2.0)] == pytest.approx(0.003, abs=1e-10)

    def test_single_segment_all_data(self):
        """
        목적: 모든 데이터가 한 구간에 속할 때 정확한 집계 검증

        Given: 금리 0~1% 범위 데이터, boundaries=[0,5]
        When: build_segment_table()
        Then: (0,5) 구간에 모든 데이터가 포함되어 정확한 mean
        """
        from qbt.tqqq.lookup_spread import build_segment_table
        ...
        realized_df = pd.DataFrame(
            {
                "ffr_pct": [0.5, 0.8],
                "realized_spread": [0.002, 0.004],
            }
        )

        table = build_segment_table(realized_df, boundaries=[0.0, 5.0], stat_func="mean")

        assert len(table) == 1
        assert table[(0.0, 5.0)] == pytest.approx(0.003, abs=1e-10)

    def test_empty_segment_excluded(self):
        """
        목적: 데이터가 없는 구간은 테이블에서 제외되는지 검증

        Given: 금리 0~2% 범위 데이터만, boundaries=[0,2,4]
        When: build_segment_table()
        Then: (2,4) 구간과 (4,inf) 구간은 테이블에 없음
        """
        from qbt.tqqq.lookup_spread import build_segment_table
        ...
        assert (2.0, 4.0) not in table
        assert (4.0, float("inf")) not in table

    def test_invalid_stat_func_raises(self):
        """
        목적: 지원하지 않는 stat_func 지정 시 ValueError 발생 검증

        Given: stat_func="mode"
        When: build_segment_table()
        Then: ValueError 발생
        """
        from qbt.tqqq.lookup_spread import build_segment_table
        ...
        with pytest.raises(ValueError, match="stat_func"):
            build_segment_table(realized_df, boundaries=[0.0, 2.0], stat_func="mode")

    def test_boundaries_must_be_ascending(self):
        """
        목적: 비오름차순 경계값 지정 시 ValueError 발생 검증

        Given: boundaries=[4.0, 2.0, 0.0] (내림차순)
        When: build_segment_table()
        Then: ValueError 발생
        """
        from qbt.tqqq.lookup_spread import build_segment_table
        ...
        with pytest.raises(ValueError, match="오름차순"):
            build_segment_table(realized_df, boundaries=[4.0, 2.0, 0.0], stat_func="mean")

    def test_boundaries_minimum_two(self):
        """
        목적: 경계값 1개 미만 시 ValueError 발생 검증 (최소 2개로 1구간)

        Given: boundaries=[0.0] (1개)
        When: build_segment_table()
        Then: ValueError 발생
        """
        from qbt.tqqq.lookup_spread import build_segment_table
        ...
        with pytest.raises(ValueError, match="최소"):
            build_segment_table(realized_df, boundaries=[0.0], stat_func="mean")


class TestLookupSpreadFromSegments:
    """구간별 스프레드 조회 테스트"""

    def test_exact_segment_lookup(self):
        """
        목적: 구간 내부 금리값에서 올바른 스프레드 반환 검증

        Given: 3구간 테이블 [(0,2)->0.003, (2,4)->0.005, (4,inf)->0.007]
        When: ffr_pct=1.0 (0~2% 구간)
        Then: 0.003 반환
        """
        from qbt.tqqq.lookup_spread import lookup_spread_from_segments
        ...
        result = lookup_spread_from_segments(
            ffr_pct=1.0,
    ...
    def test_boundary_value_at_lower(self):
        """
        목적: 구간 하한(0, 2, 4)은 해당 구간에 포함 (하한 포함, 상한 미포함) 검증

        Given: 3구간 테이블
        When: ffr_pct=2.0 (2~4% 구간의 하한)
        Then: (2,4) 구간의 0.005 반환
        """
        from qbt.tqqq.lookup_spread import lookup_spread_from_segments
    ...
    def test_last_segment_includes_infinity(self):
        """
        목적: 마지막 구간은 상한이 +inf (ffr_pct=10.0도 포함) 검증

        Given: 3구간 테이블
        When: ffr_pct=10.0 (4%+ 구간)
        Then: (4,inf) 구간의 0.007 반환
        """
        from qbt.tqqq.lookup_spread import lookup_spread_from_segments
        ...
        assert result == pytest.approx(0.007, abs=1e-10)

    def test_negative_ffr_fallback(self):
        """
        목적: ffr_pct < 0 시 첫 구간으로 fallback 검증

        Given: 3구간 테이블
        When: ffr_pct=-1.0 (음수)
        Then: 첫 구간 (0,2)의 0.003 반환
        """
        from qbt.tqqq.lookup_spread import lookup_spread_from_segments
    ...
    def test_empty_table_raises(self):
        """
        목적: 빈 테이블에서 조회 시 ValueError 발생 검증

        Given: 빈 segment_table
        When: lookup_spread_from_segments()
        Then: ValueError 발생
        """
        from qbt.tqqq.lookup_spread import lookup_spread_from_segments

        with pytest.raises(ValueError, match="비어"):
            lookup_spread_from_segments(
                ffr_pct=1.0,
                segment_table={},
                boundaries=[0.0, 2.0, 4.0],
            )
...
class TestBuildMonthlySpreadMapFromSegments:
    """구간별 스프레드 모델의 월별 스프레드 맵 생성 테스트"""

    def test_monthly_map_structure(self):
        """
        목적: FFR 데이터 3개월에서 dict[str, float] 형태 반환 검증

        Given: FFR 데이터 3개월, 3구간 테이블
        When: build_monthly_spread_map_from_segments()
        Then: dict[str, float] 형태, 3개 엔트리
        """
        from qbt.tqqq.lookup_spread import build_monthly_spread_map_from_segments

        ffr_df = pd.DataFrame({"DATE": ["2023-01", "2023-02", "2023-03"], "VALUE": [0.01, 0.03, 0.05]})
        segment_table = {
            (0.0, 2.0): 0.003,
            (2.0, 4.0): 0.005,
            (4.0, float("inf")): 0.007,
        ...
        result = build_monthly_spread_map_from_segments(
            ffr_df=ffr_df,
            segment_table=segment_table,
            boundaries=[0.0, 2.0, 4.0],
        )
        ...
        assert "2023-02" in result
        assert "2023-03" in result
        # FFR 1% → ffr_pct=1.0 → (0,2) 구간 → 0.003
        assert result["2023-01"] == pytest.approx(0.003, abs=1e-10)
        # FFR 3% → ffr_pct=3.0 → (2,4) 구간 → 0.005
        assert result["2023-02"] == pytest.approx(0.005, abs=1e-10)
        # FFR 5% → ffr_pct=5.0 → (4,inf) 구간 → 0.007
        assert result["2023-03"] == pytest.approx(0.007, abs=1e-10)

    def test_all_spreads_positive(self):
        """
        목적: 모든 스프레드 값이 양수인지 검증 (음수/0은 EPSILON 클램핑)

        Given: 음수 스프레드를 포함하는 segment_table
        When: build_monthly_spread_map_from_segments()
        Then: 모든 스프레드 값 > 0
        """
        from qbt.tqqq.lookup_spread import build_monthly_spread_map_from_segments

        ffr_df = pd.DataFrame({"DATE": ["2023-01", "2023-02"], "VALUE": [0.01, 0.03]})
        # 음수 스프레드 포함
        segment_table = {
            (0.0, 2.0): -0.001,
            (2.0, 4.0): 0.005,
