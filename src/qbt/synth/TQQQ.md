...
**2. `find_optimal_cost_model()`**
- multiplier를 3.0으로 고정
- funding_spread와 expense_ratio를 2D grid search로 캘리브레이션
- 종합 점수 기준 상위 10개 전략 선택

**3. `validate_simulation()`**
- 시뮬레이션 결과 검증
- 검증 지표 계산:
  - **기간 정보**: 시작일, 종료일, 총 일수
  - **일일 수익률**: 상관계수, 평균차이, 표준편차차이, MAE, 최대오차, RMSE
  - **로그가격**: RMSE, 최대오차
  - **누적수익률**: 실제, 시뮬, 차이, 상대차이, RMSE, 최대오차
  - **가격**: 최종차이, 일별평균차이, 일별최대차이

**4. `generate_daily_comparison_csv()`**
- 일별 상세 비교 CSV 생성
- 10개 핵심 컬럼 데이터 출력 (한글 컬럼명, 4자리 반올림)
...
2. 비용 모델 캘리브레이션 (leverage 3.0 고정, funding_spread + expense_ratio 2D grid search)
3. 상위 10개 전략 테이블 출력
4. 최적 전략으로 시뮬레이션 재실행
5. 검증 지표 계산
6. 일별 비교 CSV 생성 (`results/tqqq_daily_comparison.csv`)
7. 요약 검증 CSV 생성 - 상위 10개 전략 (`results/tqqq_validation.csv`)
8. 터미널 출력 (테이블 + 요약 통계 + 문장형 요약)
...
상위 10개 전략
============================================================
  Rank  Spread(%)  Expense(%)      Score    상관계수  RMSE(%)  최종가격차이(%)  누적수익률차이(%)
     1       0.60        0.90    0.987654    0.998900    0.2000          +0.2000            0.2000
   ...
----------------------------------------------------------------
TQQQ 시뮬레이션 검증
----------------------------------------------------------------
...
----------------------------------------------------------------
  지표                           평균         최대
  일일수익률 차이 절대값 (%)     0.0234      1.2345
  가격 차이 비율 (%)             0.5678     12.3456
  누적수익률 차이 (%)            2.34       56.78
----------------------------------------------------------------
[요약]
----------------------------------------------------------------
- 일일 수익률 상관계수 0.9989로, 실제 TQQQ의 일간 움직임을 거의 완벽하게 따라갑니다.
- 일일 수익률 RMSE는 0.20%입니다.
- 최종 가격 차이는 +0.20%로, 15.8년 이상 기간 동안 거의 동일한 수준입니다.
- 누적 수익률 상대 차이는 0.01%로, 장기 성과도 거의 완전히 일치합니다.
----------------------------------------------------------------
================================================================
```
...
검증 시 다음과 같은 지표를 계산하여 시뮬레이션 품질을 평가합니다.
...
6. **RMSE (Root Mean Squared Error)**: 일일 수익률 경로 추종 정확도
...
**누적수익률 지표**:
- **실제 누적수익률**: 실제 TQQQ 누적 수익률
- **시뮬 누적수익률**: 시뮬레이션 누적 수익률
- **차이 (퍼센트포인트)**: 누적 수익률 절대 차이
- **상대 차이 (%)**: 실제 대비 상대 오차 (검증 기준)
- **RMSE**: 누적수익률 경로 추종 정확도
- **최대 오차**: 누적수익률 최대 차이

**가격 지표**:
- **최종 가격 차이**: 마지막 날 가격 차이
- **일별 평균 차이**: 일별 가격 차이 평균
- **일별 최대 차이**: 일별 가격 차이 최대값

**CSV 출력 파일**:

1. **요약 검증 CSV** (`results/tqqq_validation.csv`):
   - 10행 × 약 27개 컬럼 (상위 10개 전략)
   - rank, leverage, funding_spread, expense_ratio, strategy_score 포함
   - 누적수익률_상대차이_pct 추가 (상대 오차)
   - 적절한 소수점 자릿수로 반올림
   - Excel 호환 인코딩 (utf-8-sig)

2. **일별 비교 CSV** (`results/tqqq_daily_comparison.csv`):
   - N행 × 10개 컬럼 (N = 검증 기간 일수)
   - 한글 컬럼명, 4자리 반올림
   - Excel 호환 인코딩 (utf-8-sig)
   - 컬럼: 날짜, 실제_종가, 시뮬_종가, 실제_일일수익률, 시뮬_일일수익률, 일일수익률_차이, 가격_차이_비율, 실제_누적수익률, 시뮬_누적수익률, 누적수익률_차이
...
### 4.3 비용 모델 캘리브레이션 알고리즘 (2D Grid Search)

**핵심 개념**: multiplier는 3.0으로 고정하고, 비용 모델 파라미터를 최적화합니다.
...
for spread in funding_spreads:
    for expense in expense_ratios:
        ...
        # 검증 지표 계산
        metrics = validate_simulation(sim_df, actual_df)

        # 종합 점수 계산
        score = compute_strategy_score(metrics)

# 3. score 기준 상위 10개 전략 선택
top_strategies = sorted_by_score[:10]
```

**점수 구성 요소**:
- **상관계수 (60%)**: 일일 수익률 패턴 유사도 - 가장 중요
- **RMSE 상대값 (25%)**: 실제 변동성 대비 일일 수익률 RMSE
- **최종 가격 차이 (10%)**: 장기 누적 결과
- **누적 수익률 상대 차이 (5%)**: 전체 수익률 일치도
...
**현재 구현**:
- **Multiplier**: 3.0 고정 (TQQQ는 3배 레버리지 ETF)
- **비용 모델 캘리브레이션**: funding_spread와 expense_ratio를 2D grid search

**검증 결과** (예시, 실행 시점에 따라 변동):
- 최적 funding_spread: 약 0.60%
- 최적 expense_ratio: 약 0.90%
- 상관계수: 0.9989 이상
...
- **누적 수익률 상대 차이**: ± 20% 이내
  - 상대 차이(%) = |시뮬 - 실제| / |실제| × 100
  - 예: 실제 26,000% vs 시뮬 26,200% → 0.77% (OK)
  - 퍼센트포인트 차이(200%p)는 보조 지표로 참고
- **최종 가격 차이**: ± 10% 이내

**검증 예시** (실행 시점에 따라 변동):
- 일일 수익률 상관계수: 0.9989 이상 ✅
- 누적 수익률 상대 차이: 1% 이내 ✅
- 최종 가격 차이: ±1% 이내 ✅
