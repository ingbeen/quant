...
설계서 3장 에 정의된 3 가지 검증을 구현한다. 본 모듈은 이상 발견 시 예외를
직접 발생시키지 않고 **에러 메시지 리스트를 반환**한다. 호출자(CLI) 가 리스트가
비어있지 않으면 즉시 중단 + 알림을 발송하는 구조이다 (설계서 11장).
...
- 보간 금지. 이상 발견 시 메시지만 반환하고 호출자가 중단 여부 결정.
- 자동 복구 없음.
...
# 설계서 2.3: 전일 종가 1% 이상 차이 시 스플릿 의심
...
def validate_prev_close(csv_close: float, yf_close: float) -> list[str]:
    ...
    설계서 2.3: CSV 의 마지막 종가와 yfinance 가 돌려주는 "같은 날짜" 의 종가가
    1% 이상 차이나면 스플릿이 의심되므로 즉시 중단한다.
